...
//        $(this).css('width', options.width);
		...
		if (cOptions.chart.type != 'pie'){
            cOptions.tooltip.formatter = cOptions.tooltip.formatter || function(){
				...
				var s = '<div style="padding:5px;"><b>' + xName + yName + '</b></div><table style="width: 150px; margin:0px; padding:0px">';    
				$.each(this.points, function(i, point) {
					...
					if(cOptions.series[i].dataFormat=='1')
						value = parseInt(value);  
					var suffix = cOptions.tooltip.valueSuffix || '';  
                    var title =  point.series.name;   
					if(cOptions.series[i].resetName)
						title=cOptions.series[i].resetName;
					
					var percentage_str ="";
					if(cOptions.series[i].percentage)
						percentage_str = ' ('+ (Math.round(this.percentage * 100) / 100) + '%)';
					...
					if(cOptions.series[i].name_hidden!='1')
					s += '<tr><td style="padding: 2px 5px" >' + title + ' </td>' 
					   + '<td style="text-align: right;padding-left:15px">' + value + suffix + percentage_str + ' </td></tr>'; 
				});           
				s += '</table>';
				return s;
			};
    ...
    //            colors: ['#49C9C3', '#FFBF3E', '#9DD30D', '#DA7D2A', '#39B54A', '#1CC4F5', '#1C95BD', '#5674B9', '#8560A8', '#9999FF'],
                  colors: ['#5D9CEC', '#62C87F', '#F15755', '#FC863F', '#7053B6', '#FFCE55', '#6ED5E6', '#F57BC1', '#DCB186', '#647C9D', '#1bd0dc', '#eb6877', '#000000', '#009944','#9DD30D','#999999'],
...
*/ 
			...
			if(cOptions.chart.type=='pie')
			hcPoint.y != (null || 0) && (hasData = true, counter++, maxY = maxY > hcPoint.y? maxY : hcPoint.y, sumY += hcPoint.y);
			else
